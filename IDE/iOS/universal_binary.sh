...
TARGET="wolfssl" # The name of the Xcode target
FILE_NAME="libwolfssl.a" # It has to match the library that is defined in the Xcode target
...
xcodebuild -project wolfssl.xcodeproj -configuration Release -target ${TARGET} -sdk ${DEVICE} ${ACTION} RUN_CLANG_STATIC_ANALYZER=NO
xcodebuild -project wolfssl.xcodeproj -configuration Release -target ${TARGET} -sdk ${SIMULATOR} ${ACTION} RUN_CLANG_STATIC_ANALYZER=NO
